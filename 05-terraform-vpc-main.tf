...
# Creates a production-grade VPC with Multi-AZ support, NAT gateways, and VPC endpoints
...
resource "aws_subnet" "private" {
  count             = var.availability_zones
  vpc_id            = aws_vpc.main.id
  cidr_block        = cidrsubnet(var.vpc_cidr, 3, count.index + var.availability_zones)
  availability_zone = data.aws_availability_zones.available.names[count.index]
...
resource "aws_subnet" "database" {
  count             = var.availability_zones
  vpc_id            = aws_vpc.main.id
  cidr_block        = cidrsubnet(var.vpc_cidr, 3, count.index + (var.availability_zones * 2))
  availability_zone = data.aws_availability_zones.available.names[count.index]
...
# Database Route Table (no direct internet access)
